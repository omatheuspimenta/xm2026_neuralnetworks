...
$$
C'_B(v) = \frac{C_B(v)}{\frac{(N-1)(N-2)}{2}} = \frac{2 \cdot C_B(v)}{(N-1)(N-2)}
$$
...
$$
\alpha_{ij} = \frac{\exp(\text{LR}(\mathbf{W_{att}}^\top [\mathbf{W}x_i \Vert \mathbf{W}x_j]))}{\sum_{k \in \mathcal{N}_i} \exp(\text{LR}(\mathbf{W_{att}}^\top [\mathbf{W}x_i \Vert \mathbf{W}x_k]))}
$$
</div>
:::
...
<div style="font-size: 0.7em;">
$$
\alpha_{ij} = \frac{\exp(\mathbf{W_{att}}^\top \text{LR}(\mathbf{W} [x_i \Vert x_j]))}{\sum_{k \in \mathcal{N}_i} \exp(\mathbf{W_{att}}^\top \text{LR}(\mathbf{W} [x_i \Vert x_k]))}
$$
</div>
:::
...
$$h_i = \sigma \left( W \left[ x_i \parallel \text{mean}_{j \in \mathcal{N}_i} (x_j) \right] \right)$$
